...
Planner 负责 orchestration：拆解任务、写入 `GlobalState`、委派隔离子任务、接收 Actor summary 和 diff、应用选中的 patch，并生成最终回复。
...
6. Actor 使用对应角色的 prompt 和工具 allowlist 执行。
...
provider 会把 MCP 子进程的当前工作目录设置为 Actor worktree，并对绝对路径做额外边界校验。不同 Actor 角色有不同 allowlist：
