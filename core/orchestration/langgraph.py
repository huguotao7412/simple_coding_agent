...
class LangGraphOrchestrator:
    ...
    def _build_graph(self, checkpointer: Any) -> Any:
        ...
        nodes = {
            "assess_task": self._assess_task,
            "compile_policy": self._compile_policy,
            "request_human_approval": self._request_approval,
            "plan": self._plan,
            "validate_plan": self._validate_plan,
            "schedule_ready_actors": self._schedule_ready_actors,
            "execute_planner": self._execute_planner,
            "execute_actor": self._execute_actor,
            "collect_actor_results": self._collect_actor_results,
            "verify": self._verify,
            "repair_router": self._repair_router,
            "bounded_repair": self._bounded_repair,
            "finalize_success": self._finalize_success,
            "finalize_failure": self._finalize_failure,
        }
        for name, handler in nodes.items():
            builder.add_node(name, cast(Any, self._instrument(name, handler)))
        ...
        builder.add_conditional_edges(
            ...
            self._collect_route,
            {
                "schedule": "schedule_ready_actors",
                "verify": "verify",
            },
        )
